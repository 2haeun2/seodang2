...
public class KitAjaxController {
	...
	//찜하기
	@RequestMapping("/kit/KitLike.do")
	@ResponseBody
	public Map<String,String> likeForm(int kit_num,HttpSession session){
		...
		if(user_num==null) {
			...
			KitLikeVO vo = kitService.selectKitLike(user_num,kit_num);
			if(vo!=null) {//이미 추천한 경우
				kitService.deleteKitLike(vo.getKitLike_num());
				map.put("result", "success");
				map.put("status", "noFav");
	//			map.put("count",kitService.selectLikeCount(kit_num)); 오류뜸
			}else{
				kitService.insertKitLike(user_num,kit_num);
				map.put("result", "success");
	...
	public Map<String, Object> execute(int kit_num,HttpSession session,KitLikeVO kitLikeVO) {
		...
		if(user_num == null) {//로그인 안됨
			...
			kitLikeVO = kitService.selectKitLike(user_num, kit_num);
