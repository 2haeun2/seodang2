...
<script type="text/javascript">
	$(function(){
		function radioText(rating){
			let radioValue = rating;
			if (radioValue == "1") {
		...
		radioText($("input[name=rating]:checked").val());
...
<div class="review">
	...
	<div class="align-center">
		<img class="review_img" src="imageView.do?off_num=${offstarVO.off_num}">
		<div>${offstarVO.off_name }</div>
	</div>
	<form:form action="offclassReviewUpdate.do" modelAttribute="offstarVO" id="review_form">
	<form:errors element="div" cssClass="error-color"/>
	<form:hidden path="offstar_num"/>
	<form:hidden path="off_num"/>
	<ul>
		<li class="starpoint_box">
			<div class="starpoint">
			<input type="radio" id="1-stars" name="rating" value="5"<c:if test="${offstarVO.rating==5 }">checked</c:if>/>
			<label for="1-stars" class="star pr-4">★</label>
			<input type="radio" id="2-stars" name="rating" value="4"<c:if test="${offstarVO.rating==4 }">checked</c:if>/>
			<label for="2-stars" class="star">★</label>
			<input type="radio" id="3-stars" name="rating" value="3"<c:if test="${offstarVO.rating==3 }">checked</c:if>/>
			<label for="3-stars" class="star">★</label>
			<input type="radio" id="4-stars" name="rating" value="2"<c:if test="${offstarVO.rating==2 }">checked</c:if>/>
			<label for="4-stars" class="star">★</label>
			<input type="radio" id="5-star" name="rating" value="1" <c:if test="${offstarVO.rating==1 }">checked</c:if>/>
